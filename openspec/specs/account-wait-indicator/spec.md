# account-wait-indicator Specification

## Purpose

The `account-wait-indicator` capability covers what an account surface shows between the moment a learner presses its button and the moment the answer arrives: a turning gold arc on the button itself, an indeterminate gold beam sweeping the top edge of the card, the fields dimmed and made inert, one sentence naming the work, and exactly one live region that says it.

It exists because the only acknowledgement these surfaces used to give was a swapped button label. Nothing moved, so a slow Turnstile check or an irregular network left the learner looking at a form that appeared not to have heard them — and the usual answer to a page that looks idle is to press the button again. The wait is the same on every account surface, so it is defined once and reused: a form's submit button and the profile page's delete button wait identically.

Everything here is indeterminate by construction. The beam and the arc say "still happening" and nothing more: no percentage, no stage, no step count, no estimated time, because nothing the client can observe would support one. What survives `prefers-reduced-motion` — the dimmed inert region, the pending label, the status line — is the part that carries the meaning.

The surfaces that use it are specified in `learner-account`. The ubiquitous language is `GLOSSARY.md`.
