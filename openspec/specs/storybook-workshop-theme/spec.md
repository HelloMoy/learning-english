# storybook-workshop-theme Specification

## Purpose

The `storybook-workshop-theme` capability covers how Storybook presents itself: the manager in
Immersion Cinema dark under the `ENGLISH·COURSE` brand, stories opened in the app's default
variant on the app's own backdrop, and a `Docs` group that opens the workshop as the design
system for English Course and obeys the toolbar's theme and locale.

It exists because Storybook is where every component is reviewed. A review surface that looks
unlike the app, opens in the variant the learner never sees first, or ignores the controls on its
own toolbar lets a component pass review against the wrong background. Whether a story can render
at all is `storybook-preview-integrity`'s concern, not this one's.
