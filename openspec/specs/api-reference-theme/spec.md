# api-reference-theme Specification

## Purpose
Define how the API reference that `pnpm run docs` generates with TypeDoc looks and what its home page shows. The reference wears the app's Immersion Cinema dark theme through a local TypeDoc plugin (`scripts/typedoc-cinema-theme/`): the cinema palette whatever the reader's OS or stored preference, the `ENGLISH·COURSE` wordmark, a home page that starts from the hexagon (layer posters with live module counts and a module index grouped by folder), declaration pages with a kind eyebrow and a gold-framed signature, and pages that never serve a stale build.
## Requirements
