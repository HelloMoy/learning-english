# ui-skeleton-primitive Specification

## Purpose

The `ui-skeleton-primitive` capability covers shadcn's `Skeleton` component: the one pulsing block every loading placeholder in the app is composed from, its accessibility contract, and the artifacts the project requires of any reusable component.

It is deliberately thin. The primitive has no size of its own and no voice — callers shape it through `className`, and announcing that something is loading belongs to the single live region its containing shell owns, never to the individual shapes. A shell built from a dozen skeletons must announce "loading" once, not twelve times.

Its consumers are specified in `loading-skeletons`.
