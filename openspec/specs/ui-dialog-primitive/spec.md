# Capability: ui-dialog-primitive

## Purpose

The `ui-dialog-primitive` capability owns the project's shadcn/ui `Dialog` primitive and the `NiceModal` + `Dialog` composition contract that every modal in the application must follow. It covers the primitive's location and exports, its accessible structure (portal, overlay, focus trap, `Escape` and backdrop dismissal, required title and description), its theming on the project's own design tokens rather than stock shadcn neutrals, and the imperative `NiceModal.show(...)` trigger that replaces prop-drilled `open` booleans and conditionally rendered modal children.

`AGENTS.md` §Modals already mandates `@ebay/nice-modal-react` paired with a shadcn `<Dialog>`, and `<NiceModal.Provider>` is mounted in `global-providers.tsx`. This capability makes that convention buildable and enforceable: the primitive exists, it is built on the `radix-ui` unified package already used by `button.tsx` (no new dependency), and its structure is kept recognisably close to the shadcn registry source so a future `shadcn diff` stays readable.
