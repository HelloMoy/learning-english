...
### Requirement: The guide teaches the four taps iOS actually requires

The app SHALL offer a guide that instructs an iPhone Safari learner how to add the course to
their home screen, as the four steps iOS 26 requires, in order:

1. the **«···»** control in Safari's bottom bar,
2. **Share** in the menu that opens,
3. **Add to Home Screen** in the share sheet,
4. **Add** on the confirmation screen.
...
#### Scenario: The steps are the four iOS requires, in order

- **WHEN** the guide is rendered
- **THEN** it presents exactly four steps, in the order iOS presents them, the first naming
...
The only control the guide SHALL offer is the one that dismisses it.
...
screen among the learner's other apps. The four taps are the cost; this is what they buy, and
a guide that stops at the confirmation screen asks for effort without ever showing the payoff.
...
- **THEN** it carries no step number, and the steps still count only the four taps
