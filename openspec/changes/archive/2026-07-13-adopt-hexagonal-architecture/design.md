...
**Choice**: `eslint.config.mjs` gains a block scoped to `src/domain/**` with `no-restricted-imports` whose `patterns` is `["*"]` and `allow` is exactly `["zod", "neverthrow"]`. The shipped implementation uses a denylist of every delivery-mechanism package and its sub-paths (via both `paths` for bare specifiers and `patterns` for sub-paths like `next/navigation`, `@/adapters/...`, etc.) because ESLint v9's `no-restricted-imports` has no native `allow` mode in flat config; the closed set is the contract regardless of the mechanism. Plus a `no-restricted-syntax` block forbidding any `new Date(...)`, `Date.now()`, `Date.UTC()`, `Math.random()`, `crypto.*`, and `globalThis.crypto.*` calls inside `src/domain/**`.
...
**Note** (recorded at archive time): The follow-up change `add-lesson-view` extended the seed to one video lesson + two reading lessons across two modules, in order to exercise the `VideoLesson` variant and the `Course → Module → Lesson` hierarchy. The shape of the in-memory adapters stayed the same; only the seed contents evolved.
