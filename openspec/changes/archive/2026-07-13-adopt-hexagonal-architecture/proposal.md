...
- Configure ESLint with an **allowlist** rule (`no-restricted-imports`) for `src/domain/**` that allows only `zod` and `neverthrow`. Adding any other dependency requires updating the spec.
