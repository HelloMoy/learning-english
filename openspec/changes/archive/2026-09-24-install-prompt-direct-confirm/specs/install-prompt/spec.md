...
prompt SHALL be decided after hydration and SHALL be treated as unavailable until the event
has actually arrived. A browser that never fires the event — Firefox, Safari on any platform —
SHALL therefore never reach the prompt.

The default action of `beforeinstallprompt` SHALL be prevented when it fires, so that the
browser does not show its own promotion banner alongside this one, and the event SHALL be
retained so that it can be used later from the learner's own gesture.

#### Scenario: The event has fired and the app is not installed

- **WHEN** `beforeinstallprompt` has fired and the app is not running standalone
- **THEN** the install prompt is available
...
The prompt SHALL present the application's identity, one statement of what the learner gains,
and exactly two controls: one that proceeds with the install and one that dismisses.

Activating the proceeding control SHALL call the retained event's own prompt method, from
within the learner's gesture, and SHALL close the modal. What the learner sees next is the
browser's own install dialog; the modal SHALL NOT present itself as having installed anything,
and SHALL NOT report success, because the outcome belongs to a dialog this application does not
draw.
...
- **THEN** no success message is shown, and the application does not assert the app was
  installed
