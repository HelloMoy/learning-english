# Delta Spec — course-platform-domain

This is a delta spec on top of the capability defined in
`openspec/changes/adopt-hexagonal-architecture/specs/course-platform-domain/spec.md`.
It extends the domain with `Module`, `VideoLesson`, `Resource`, new ports, and new
use cases for the Lesson Page. The full text of the prior spec is the baseline;
`## MODIFIED Requirements` show the updated form of an existing requirement
(full block, per the OpenSpec workflow), and `## ADDED Requirements` introduce
new behavior.
...
- **WHEN** any input is passed to any use case (valid, invalid, boundary)
...
The domain SHALL access collaborators (data, time, identity, randomness) exclusively through port interfaces defined under `src/domain/ports/**`. The ports SHALL include:
...
Implementations of these ports SHALL live outside `src/domain/**`. The implementations SHALL be in-memory adapters under `src/adapters/persistence/in-memory/**`.
...
- **WHEN** any implementation of `CourseRepository`, `LessonRepository`, `ModuleRepository`, `ResourceRepository`, `Clock`, or `IdGenerator` is queried
...
The `markLessonComplete` use case SHALL write to a `ProgressTracker` port (or to an existing port extended for this purpose) backed by an in-memory store in the v1 adapter. The completed state SHALL be lost when the in-memory adapter is reconstructed. This is the v1 limitation; the use case contract is unchanged when durable storage arrives.
...
#### Scenario: Completed state is lost when the adapter is reconstructed
- **WHEN** `markLessonComplete` is called, then the in-memory adapter is rebuilt (e.g., server restart)
- **THEN** the next call against the rebuilt adapter still resolves to `{ ok: true, value: { completed: true } }` but the previous "completed" state is no longer observable to the consumer (the contract is per-call, not persisted)
