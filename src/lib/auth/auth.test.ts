...
describe.skipIf(!DOCKER_AVAILABLE)("createAuth (integration)", () => {
  ...
  function post(
    path: string,
    body: unknown,
    init: { cookie?: string; token?: string | null } = {},
  ) {
  ...
  function signIn(learner: Pick<Learner, "email" | "password">, token?: string | null) {
    return post("/sign-in/email", { email: learner.email, password: learner.password }, { token });
  }
