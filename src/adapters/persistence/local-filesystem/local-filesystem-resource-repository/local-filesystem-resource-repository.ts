 ...
 * and resolution happens here at read time rather than when the manifest is
 * written, so
 * the storage backend can change without touching the course manifests.
 */
export class LocalFilesystemResourceRepository implements ResourceRepository {
  ...
  listByModule(moduleId: ModuleId): Promise<Resource[]> {
    ...
    // manifest doesn't carry it directly on the Resource.
    void moduleId;
